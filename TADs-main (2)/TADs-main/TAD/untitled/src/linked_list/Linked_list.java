...
public class Linked_list<T> implements linked_list_interface<T>, queue.queue_interfaz<T>, stack.stack_interfaz<T> {
    ...
    public T get(int posicion) throws IndexOutOfBoundsException {
        if (posicion < 0 || posicion >= size()) {
            ...
            while (currentNode != null && currentPosition != posicion) {
    ...
    private T removeFirst() { // esta operación remueve el primer elemento y retorna el elemento eliminado
        T valueToRemove = null;

        if (this.first != null) {
            valueToRemove = this.first.getValue();
